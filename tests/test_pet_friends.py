...
def test_add_pet_photo(pet_photo='images/dog.jpg'):
    """Проверяем, что фото питомца можно добавить"""
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
...
def test_get_my_pets_with_invalid_key(filter="my_pets"):
    ...
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.get_list_of_pets({'key': 'dbfc699ae3e7409cca4acb054f3433ebb834566'}, filter)
    assert status == 403


def test_add_new_pet_with_negative_age(name='Барсик', animal_type='кот',
                                       age='-1', pet_photo='images/siberian-cat-2.jpg'):
